...
class UpdateTicketCategoryRequest extends FormRequest
{
    ...
    {
        $ticketCategoryId = (int) ($this->route('ticketCategory')?->id ?? $this->route('ticket_category')?->id);

        return [
            'code' => ['required', 'string', 'max:50', Rule::unique('ticket_categories', 'code')->ignore($ticketCategoryId)],
