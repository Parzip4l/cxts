...
namespace App\Modules\Tickets\TicketDetailSubcategories\Requests;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class UpdateTicketDetailSubcategoryRequest extends FormRequest
{
    ...
    {
        $ticketDetailSubcategoryId = (int) ($this->route('ticketDetailSubcategory')?->id ?? $this->route('ticket_detail_subcategory')?->id);

        return [
            'ticket_subcategory_id' => ['required', 'integer', Rule::exists('ticket_subcategories', 'id')],
            'code' => [
                'required',
                'string',
                'max:50',
                Rule::unique('ticket_detail_subcategories', 'code')
                    ->where(fn ($query) => $query->where('ticket_subcategory_id', $this->input('ticket_subcategory_id')))
                    ->ignore($ticketDetailSubcategoryId),
            ],
            ...
            'engineer_skill_ids.*' => ['integer', Rule::exists('engineer_skills', 'id')],
